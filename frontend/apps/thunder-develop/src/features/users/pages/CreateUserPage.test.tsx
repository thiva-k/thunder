...
interface UseCreateUserReturn {
  createUser: (data: {organizationUnit: string; type: string; attributes: Record<string, unknown>}) => Promise<CreateUserResponse>;
...
describe('CreateUserPage', () => {
  ...
  it('renders Create button for user type', () => {
    ...
    const createTypeButton = createButtons.find(button => button.textContent?.includes('Create') && button !== screen.getByRole('button', {name: /create user/i}));
  ...
  it('logs success message when user is created', async () => {
    const consoleSpy = vi.spyOn(console, 'log').mockImplementation(() => undefined);
    const user = userEvent.setup();
    const mockResponse: CreateUserResponse = {
      id: 'user123',
    ...
    await waitFor(() => {
      expect(consoleSpy).toHaveBeenCalledWith('User created successfully:', mockResponse);
    });
