...
export default function VerifiablePresentationForm({
  ...
  const dirty =
    snapshot(handle, effectiveOuId, displayName, vct, format, claims, enforceTrustedIssuer, trustedAuthorities) !==
    initialSnapshot;
  ...
  return (
    <Stack spacing={3}>
      <Tabs value={tab} onChange={(_e: SyntheticEvent, v: number): void => setTab(v)} aria-label="presentation definition">
      ...
      <TabPanel value={tab} index={0}>
        <Stack spacing={3}>
          {definitionId && (
            <SettingsCard
              title={t('form.quickCopy.title')}
              description={t('form.quickCopy.description')}
            >
          ...
          <SettingsCard
            ...
            description={t('form.details.description')}
          >
            <Stack spacing={3}>
          ...
          {definitionId && onDelete && (
            <SettingsCard
              title={t('form.dangerZone.title')}
              description={t('form.dangerZone.description')}
            >
      ...
      <TabPanel value={tab} index={2}>
        <SettingsCard
          title={t('form.issuerTrust.title')}
          description={t('form.issuerTrust.description',
          )}
        >
          <Stack spacing={3}>
            <Box>
              ...
              />
              <FormHelperText>
                {t('form.issuerTrust.enforce.hint',
                )}
              </FormHelperText>
            </Box>
            <FormControl fullWidth>
              <FormLabel htmlFor="vp-trusted-authorities">
                {t('form.issuerTrust.authorities.label')}
              </FormLabel>
              <Autocomplete<TrustAnchor, true, false, false>
                ...
                value={trustAnchors.filter((anchor: TrustAnchor): boolean =>
                  trustedAuthorities.includes(anchor.name),
                )}
