...
vi.mock('@/features/flows/components/resources/steps/CommonStaticStepFactory', () => ({
  CommonStaticStepFactory: () => <div data-testid="static-step-factory">Static Step Factory</div>,
}));
...
vi.mock('@/features/flows/api/useGetFlowBuilderResources', () => ({
  default: () => ({
    data: {
...
describe('processFormComponents - Executor Assignment Logic', () => {
  it('should assign PASSWORD_PROVISIONING executor when password field exists with single submit', () => {
    ...
    const executorName = hasPasswordField
      ...
      : FlowConstants.ExecutorNames.EMAIL_OTP;

    expect(submitButtonCount === 1 && (hasPasswordField || hasOtpField)).toBe(true);
    expect(executorName).toBe(FlowConstants.ExecutorNames.PASSWORD_PROVISIONING);
  });
  ...
  it('should assign EMAIL_OTP executor when OTP field exists with single submit', () => {
    ...
    expect(submitButtonCount === 1 && (hasPasswordField || hasOtpField)).toBe(true);
    expect(executorName).toBe(FlowConstants.ExecutorNames.EMAIL_OTP);
  });
  ...
  it('should update action with executor and type when conditions are met', () => {
    ...
    const executorName = FlowConstants.ExecutorNames.PASSWORD_PROVISIONING;
    ...
    expect(updatedComponent.action.executor.name).toBe(FlowConstants.ExecutorNames.PASSWORD_PROVISIONING);
    expect(updatedComponent.action.type).toBe(FlowConstants.ActionTypes.EXECUTOR);
  });
});
...
describe('StepsByType Ref Update Effect - Component Integration', () => {
  ...
  it('should render component which triggers steps indexing effect', () => {
    ...
    // The effect runs when steps change (useGetFlowBuilderResources mock provides steps)
    expect(screen.getByTestId('flow-builder')).toBeInTheDocument();
  });
});
