...
export default function VerifiableCredentialForm({
  ...
  const initialSnapshot = useMemo(() => {
    ...
    return snapshot({
      ...
      display: dn || loc || logo ? {name: dn || undefined, locale: loc || undefined, logoUri: logo || undefined} : undefined,
    });
  }, [initial]);
  ...
  return (
    <Stack spacing={3}>
      <Tabs value={tab} onChange={(_e: SyntheticEvent, v: number): void => setTab(v)} aria-label="credential configuration">
