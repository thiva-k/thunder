...
| 1.2 | 2026-09-06 | Redrew the architecture and sequence diagrams against the real database layout (`config`, `entity`, `runtime_transient`, `runtime_persistent`) and a consistent level of detail across grants. Restructured Residual risks into a table and narrowed it to the one gap without a separate tracking issue: incomplete criteria-based revocation on role, credential, group, and application changes. Extended that threat to `[02]-9` and `[04]-9`, alongside the existing `[03]-3`, since it applies to every grant's issued access tokens, not only `client_credentials`. Removed issue-number and tracking references from the threat-assessment tables; that information now lives only in the Residual risks table. |
| 1.3 | 2026-09-06 | Added a second Residual risks row for missing standardized audit logging, and pointed Security Review Checklist items 13 and 14 at it instead of embedding the tracking issue directly in their Comments cells, consistent with how the threat-assessment tables reference the table. |
