...
| --- | --- | --- | --- |
| Stale authorization on access tokens and authorization codes after an entitlement change | Criteria-based revocation exists, but only the token-family and subject dimensions have a writer and are enforced on the hot path. Role and group changes are re-evaluated at every refresh exchange, and a credential change rejects the refresh outright, so the residual is the outstanding access token, and an unredeemed authorization code, which both keep their stale authorization until expiry. Most acute for `client_credentials`, where no writer records the application, role, or group dimensions. | Accepted. Added to the criteria-based revocation plan following review. | Interim mitigation is short TTLs and explicit revocation by `jti`. Extend the criteria-based revocation writer and hot-path enforcement to the role, group, and application dimensions for access tokens and authorization codes. See the Token and Protocol Features model [02]-3 and the Token Revocation threat model. (tracking: TBD, see the broader criteria-based revocation initiative thunder-id/thunderid#4127) |
