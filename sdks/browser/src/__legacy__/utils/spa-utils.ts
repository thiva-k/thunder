...
import {ThunderIDAuthClient, OIDCRequestConstants} from '@thunderid/javascript';
...
export class SPAUtils {
  ...
  public static async isSignOutSuccessful(): Promise<boolean> {
    if (ThunderIDAuthClient.isSignOutSuccessful(window.location.href)) {
      const newUrl = window.location.href.split('?')[0];
      history.pushState({}, document.title, newUrl);

      await ThunderIDAuthClient.clearSession();
  ...
  public static didSignOutFail(): boolean | SignOutError {
    if (ThunderIDAuthClient.didSignOutFail(window.location.href)) {
   ...
   * @since 0.2.0
   * @deprecated Use `hasAuthParamsInUrl` from `@thunderid/browser` instead.
   *
   ...
   */
  public static until = (condition: () => boolean, timeout = 500) => {
