...
import {ThunderIDNodeCore} from './core';
 ...
 *
 * @export
 * @class ThunderIDNodeClient
 */
export class ThunderIDNodeClient<T> {
  private authCore: ThunderIDNodeCore<T>;

  /**
    * This is the constructor method that returns an instance of the `ThunderIDNodeClient` class.
    *
    ...
    * const auth = new ThunderIDNodeClient(_config,_store);
    * ```
    *
  ...
  public async initialize(config: AuthClientConfig<T>, store?: Storage): Promise<boolean> {
    this.authCore = new ThunderIDNodeCore(config, store);
   ...
   * @param {string} authorizationCode - The authorization code obtained from ThunderID after a user signs in.
   * @param {String} sessionState - The session state obtained from ThunderID after a user signs in.
  ...
  /**
   * This method can be used to update the configurations passed into the constructor of the ThunderIDAuthClient.
  ...
  public static isSignOutSuccessful(afterSignOutUrl: string): boolean {
    return ThunderIDNodeClient.isSignOutSuccessful(afterSignOutUrl);
  }
  ...
  public static didSignOutFail(afterSignOutUrl: string): boolean {
    return ThunderIDNodeClient.didSignOutFail(afterSignOutUrl);
  }
