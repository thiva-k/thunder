...
// Users only have system credentials (password, passkey, TOTP), so schema credentials are nil.
...
func makeUserValidator() func(e *entity.Entity, svc entity.EntityServiceInterface) error {
	return func(e *entity.Entity, svc entity.EntityServiceInterface) error {
		...
		_, err := svc.GetEntity(context.Background(), e.EntityID)
		if err == nil {
			return fmt.Errorf("duplicate user ID '%s': user already exists", e.EntityID)
		}
		if !errors.Is(err, entity.ErrEntityNotFound) {
			return fmt.Errorf("checking user existence for '%s': %w", e.EntityID, err)
		}
