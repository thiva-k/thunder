...
func (f *definitionFileBasedStore) Create(_ context.Context, dto PresentationDefinitionDTO) error {
...
func (f *definitionFileBasedStore) GetByID(_ context.Context, id string) (*PresentationDefinitionDTO, error) {
...
func (f *definitionFileBasedStore) GetByHandle(
...
func (f *definitionFileBasedStore) ListSummaries(_ context.Context) ([]PresentationDefinitionSummary, error) {
	list, err := f.GenericFileBasedStore.List()
	if err != nil {
		return nil, err
	}
	summaries := make([]PresentationDefinitionSummary, 0, len(list))
...
func (f *definitionFileBasedStore) List(_ context.Context) ([]PresentationDefinitionDTO, error) {
	list, err := f.GenericFileBasedStore.List()
	if err != nil {
		return nil, err
	}
...
func (f *definitionFileBasedStore) Update(_ context.Context, _ PresentationDefinitionDTO) error {
	return ErrDefinitionIsImmutable
}

// Delete is not supported in the file-based store.
func (f *definitionFileBasedStore) Delete(_ context.Context, _ string) error {
	return ErrDefinitionIsImmutable
}

// IsDeclarative reports whether the given id exists in the file-based store.
func (f *definitionFileBasedStore) IsDeclarative(_ context.Context, id string) (bool, error) {
