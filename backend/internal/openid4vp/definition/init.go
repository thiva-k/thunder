...
func Initialize(
	...
	svc := newPresentationDefinitionService(store, ouResolver)
...
func initializeStore() (definitionStoreInterface, error) {
	storeMode, err := getPresentationDefinitionStoreMode()
	if err != nil {
		return nil, err
	}
...
// getPresentationDefinitionStoreMode determines the store mode for presentation definitions.
//
// Resolution order:
...
func getPresentationDefinitionStoreMode() (serverconst.StoreMode, error) {
