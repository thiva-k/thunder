...
// PresentationDefinitionSummary is the minimal projection returned by the list
// endpoint. It contains only the fields the management UI renders in the table.
type PresentationDefinitionSummary struct {
	ID          string `json:"id"`
	Handle      string `json:"handle"`
	OUID        string `json:"ouId"`
	OUHandle    string `json:"ouHandle,omitempty"`
...
func toSummary(dto PresentationDefinitionDTO) PresentationDefinitionSummary {
	return PresentationDefinitionSummary{
		ID:          dto.ID,
		Handle:      dto.Handle,
		OUID:        dto.OUID,
