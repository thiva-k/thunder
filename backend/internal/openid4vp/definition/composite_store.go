...
func (c *compositeDefinitionStore) Create(ctx context.Context, dto PresentationDefinitionDTO) error {
...
func (c *compositeDefinitionStore) GetByID(ctx context.Context, id string) (*PresentationDefinitionDTO, error) {
	return declarativeresource.CompositeGetHelper(
		func() (*PresentationDefinitionDTO, error) { return c.dbStore.GetByID(ctx, id) },
		func() (*PresentationDefinitionDTO, error) { return c.fileStore.GetByID(ctx, id) },
		ErrNotFound,
	)
}
...
// Checks the database store first, then falls back to the file store.
func (c *compositeDefinitionStore) GetByHandle(
	ctx context.Context, handle string,
) (*PresentationDefinitionDTO, error) {
	return declarativeresource.CompositeGetHelper(
		func() (*PresentationDefinitionDTO, error) { return c.dbStore.GetByHandle(ctx, handle) },
		func() (*PresentationDefinitionDTO, error) { return c.fileStore.GetByHandle(ctx, handle) },
		ErrNotFound,
	)
}

// ListSummaries retrieves minimal listing data from both stores and merges the results.
func (c *compositeDefinitionStore) ListSummaries(ctx context.Context) ([]PresentationDefinitionSummary, error) {
	...
	result := make([]PresentationDefinitionSummary, 0, len(dbSummaries)+len(fileSummaries))
...
func (c *compositeDefinitionStore) List(ctx context.Context) ([]PresentationDefinitionDTO, error) {
	dbDefs, err := c.dbStore.List(ctx)
	if err != nil {
		return nil, err
	}
	fileDefs, err := c.fileStore.List(ctx)
	if err != nil {
		return nil, err
	}
...
func (c *compositeDefinitionStore) Update(ctx context.Context, dto PresentationDefinitionDTO) error {
	return declarativeresource.CompositeUpdateHelper(
		...
		func(d PresentationDefinitionDTO) error { return c.dbStore.Update(ctx, d) },
...
// Returns ErrDefinitionIsImmutable if the definition is declarative (exists in file store).
func (c *compositeDefinitionStore) Delete(ctx context.Context, id string) error {
...
func (c *compositeDefinitionStore) IsDeclarative(ctx context.Context, id string) (bool, error) {
