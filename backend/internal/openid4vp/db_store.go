...
// dbStateStore persists OpenID4VP request state in the runtime database so it
...
func newDBStateStore(crypto kmprovider.ConfigCryptoProvider) stateStore {
	return &dbStateStore{
...
func (s *dbStateStore) Save(ctx context.Context, st *RequestState) error {
...
func (s *dbStateStore) Get(ctx context.Context, state string) (*RequestState, bool) {
...
func (s *dbStateStore) Delete(ctx context.Context, state string) error {
	dbClient, err := s.dbProvider.GetRuntimeDBClient()
	if err != nil {
		return fmt.Errorf("failed to get runtime database client: %w", err)
	}
...
func (s *dbStateStore) buildRequestStateFromRow(
