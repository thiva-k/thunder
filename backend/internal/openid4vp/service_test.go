...
// fakeDefinitionReader is an in-memory definitionReader for verifier tests.
type fakeDefinitionReader struct {
	byHandle map[string]definition.PresentationDefinitionDTO
}

func newFakeDefinitionReader() *fakeDefinitionReader {
	return &fakeDefinitionReader{byHandle: map[string]definition.PresentationDefinitionDTO{}}
}

func (f *fakeDefinitionReader) GetByHandle(
	_ context.Context, handle string,
) (*definition.PresentationDefinitionDTO, *serviceerror.ServiceError) {
	dto, ok := f.byHandle[handle]
	if !ok {
...
func newTestService(t *testing.T, b *pidBuilder) (*service, *fakeStore) {
	...
	defStore := newFakeDefinitionReader()
	defStore.byHandle[testDefinitionID] = definition.PresentationDefinitionDTO{
		ID:              testDefinitionID,
	...
	svc, err := newOpenID4VPService(cfg, store, testAudience, &fakeSigner{key: signerKey}, b.trustStore(), defStore)
	require.NoError(t, err)
...
func (suite *OpenID4VPServiceTestSuite) TestRequestObjectBuildsSignedJAR() {
	...
	jar, svcErr := svc.RequestObject(context.Background(), init.State)
	suite.Require().Nil(svcErr)
	...
	suite.Equal(store.m[init.State].Nonce, claims["nonce"])
...
func (suite *OpenID4VPServiceTestSuite) TestRequestObjectUnknownState() {
	...
	_, svcErr := svc.RequestObject(context.Background(), "nope")
...
func (suite *OpenID4VPServiceTestSuite) TestSubmitResponseHappyPath() {
	...
	res, svcErr := svc.Result(context.Background(), init.State)
	suite.Require().Nil(svcErr)
...
func (suite *OpenID4VPServiceTestSuite) TestSubmitResponseRestrictsTrustedAuthority() {
	...
	enforceIssuer := true
	def := svc.defStore.(*fakeDefinitionReader).byHandle[testDefinitionID]
	def.EnforceTrustedIssuer = &enforceIssuer
	def.TrustedAuthorities = []string{"unrelated-root"}
	svc.defStore.(*fakeDefinitionReader).byHandle[testDefinitionID] = def
	...
	suite.Equal(StatusFailed, store.m[init.State].Status)
}
...
func (suite *OpenID4VPServiceTestSuite) TestSubmitResponseWrongNonceMarksFailed() {
	...
	suite.Equal(ErrorVerificationFailed.Code, svcErr.Code)
	suite.Equal(StatusFailed, store.m[init.State].Status)
	suite.NotEmpty(store.m[init.State].FailureReason)
}
...
func (suite *OpenID4VPServiceTestSuite) TestExpiredStateRejected() {
	...
	_, svcErr = svc.Result(context.Background(), init.State)
	suite.Require().NotNil(svcErr)
	suite.Equal(ErrorUnknownState.Code, svcErr.Code)
...
func (suite *OpenID4VPServiceTestSuite) TestResultRedirectURI() {
	...
	suite.Equal("", svc.ResultRedirectURI("some-state"))
...
func (suite *OpenID4VPServiceTestSuite) TestAuthenticate_CompletedValidPresentation() {
	...
	suite.Equal("https://issuer.example", result.Token["openid4vp_issuer"])
	suite.Equal(testVCT, result.Token["openid4vp_vct"])
	suite.Equal("Ada", result.Token["given_name"])
	suite.Equal("Lovelace", result.Token["family_name"])
	// AuthenticatedClaims is the same map as Token
	suite.Equal(result.Token, result.AuthenticatedClaims)
}
...
func (suite *OpenID4VPServiceTestSuite) TestAuthenticate_CompletedNoSubject() {
	...
	suite.Equal("DE123456", result.Token["tax_id"])
}
