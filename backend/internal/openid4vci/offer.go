...
// Offer builds and stores an issuer-initiated credential offer for configID and
// returns it with the openid-credential-offer:// deep link (offer by reference).
func (s *service) Offer(ctx context.Context, configID string) (map[string]interface{}, string, error) {
	cred, svcErr := s.creds.GetByHandle(ctx, configID)
	if svcErr != nil {
	...
	if err := s.offers.Save(ctx, rec); err != nil {
...
// CredentialOffer returns a previously stored issuer-initiated credential offer by
// id, so a wallet can resolve the credential_offer_uri.
func (s *service) CredentialOffer(ctx context.Context, id string) (map[string]interface{}, error) {
	rec, ok := s.offers.Get(ctx, id)
	if !ok || rec == nil {
