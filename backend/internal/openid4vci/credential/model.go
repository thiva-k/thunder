...
// openid4vci package reads these configurations on demand via the configurationStoreInterface.
package credential
...
// CredentialConfigurationSummary is the minimal projection returned by the list
// endpoint. It contains only the fields the management UI renders in the table.
type CredentialConfigurationSummary struct {
	ID          string `json:"id"`
	Handle      string `json:"handle"`
	OUID        string `json:"ouId"`
	OUHandle    string `json:"ouHandle,omitempty"`
...
func toConfigSummary(dto CredentialConfigurationDTO) CredentialConfigurationSummary {
	displayName := ""
	if dto.Display != nil {
		displayName = dto.Display.Name
	}
	return CredentialConfigurationSummary{
		ID:          dto.ID,
		Handle:      dto.Handle,
		OUID:        dto.OUID,
