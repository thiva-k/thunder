...
func (s *DBStoreTestSuite) TestGetGroupCountForEntity_ProviderError() {
	s.expectClientError()
	_, err := s.store.GetGroupCountForEntity(s.ctx, "e1", "user")
	s.Error(err)
}
...
func (s *DBStoreTestSuite) TestGetEntityGroups_ProviderError() {
	s.expectClientError()
	_, err := s.store.GetEntityGroups(s.ctx, "e1", "user", 10, 0)
	s.Error(err)
}
...
func (s *DBStoreTestSuite) TestGetEntityGroups_Success() {
	...
	groups, err := s.store.GetEntityGroups(s.ctx, "e1", "user", 10, 0)
	s.NoError(err)
	s.Len(groups, 1)
