...
var (
	...
	QueryGetGroupCountForEntity = model.DBQuery{
		...
		Query: `SELECT COUNT(*) AS total FROM GROUP_MEMBER_REFERENCE ` +
			`WHERE MEMBER_ID = $1 AND MEMBER_TYPE = $2 AND DEPLOYMENT_ID = $3`,
	}
	...
	QueryGetGroupsForEntity = model.DBQuery{
		...
		Query: `SELECT G.ID, G.OU_ID, G.NAME FROM GROUP_MEMBER_REFERENCE GMR ` +
			`INNER JOIN "GROUP" G ON GMR.GROUP_ID = G.ID AND GMR.DEPLOYMENT_ID = $5 AND G.DEPLOYMENT_ID = $5 ` +
			`WHERE GMR.MEMBER_ID = $1 AND GMR.MEMBER_TYPE = $2 AND GMR.DEPLOYMENT_ID = $5 ` +
