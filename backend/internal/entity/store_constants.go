...
var (
	...
	QueryGetEntityCount = model.DBQuery{
		...
		Query: "SELECT COUNT(*) as total FROM ENTITY WHERE ENTITY_CATEGORY = $1 AND DEPLOYMENT_ID = $2",
	}
	...
	QueryGetEntityList = model.DBQuery{
		...
		Query: "SELECT ENTITY_ID, OU_ID, ENTITY_CATEGORY, ENTITY_TYPE, STATE, ATTRIBUTES, SYSTEM_ATTRIBUTES FROM ENTITY " +
			"WHERE ENTITY_CATEGORY = $4 AND DEPLOYMENT_ID = $3 ORDER BY ENTITY_ID LIMIT $1 OFFSET $2",
	}
	...
	QueryCreateEntity = model.DBQuery{
		...
		Query: "INSERT INTO ENTITY " +
			"(ENTITY_ID, DEPLOYMENT_ID, ENTITY_CATEGORY, ENTITY_TYPE, STATE, OU_ID, " +
	...
	QueryGetEntityByID = model.DBQuery{
		...
		Query: "SELECT ENTITY_ID, OU_ID, ENTITY_CATEGORY, ENTITY_TYPE, STATE, ATTRIBUTES, SYSTEM_ATTRIBUTES " +
			"FROM ENTITY WHERE ENTITY_ID = $1 AND DEPLOYMENT_ID = $2",
	}
	// QueryUpdateEntity is the query to update an entity.
	QueryUpdateEntity = model.DBQuery{
		ID: "ASQ-ENTITY_MGT-05",
		Query: "UPDATE ENTITY SET OU_ID = $2, ENTITY_TYPE = $3, ATTRIBUTES = $4 " +
	...
	QueryUpdateSystemAttributes = model.DBQuery{
		...
		Query: "UPDATE ENTITY SET SYSTEM_ATTRIBUTES = $2 WHERE ENTITY_ID = $1 AND DEPLOYMENT_ID = $3",
	}
	// QueryUpdateCredentials is the query to update credentials.
	QueryUpdateCredentials = model.DBQuery{
		ID:    "ASQ-ENTITY_MGT-07",
		Query: "UPDATE ENTITY SET CREDENTIALS = $2 WHERE ENTITY_ID = $1 AND DEPLOYMENT_ID = $3",
	}
	...
	QueryUpdateSystemCredentials = model.DBQuery{
		...
		Query: "UPDATE ENTITY SET SYSTEM_CREDENTIALS = $2 WHERE ENTITY_ID = $1 AND DEPLOYMENT_ID = $3",
	}
	// QueryDeleteEntity is the query to delete an entity.
	QueryDeleteEntity = model.DBQuery{
		ID:    "ASQ-ENTITY_MGT-09",
		Query: "DELETE FROM ENTITY WHERE ENTITY_ID = $1 AND DEPLOYMENT_ID = $2",
	}
	...
	QueryGetEntityWithCredentials = model.DBQuery{
		...
		Query: "SELECT ENTITY_ID, OU_ID, ENTITY_CATEGORY, ENTITY_TYPE, STATE, ATTRIBUTES, " +
	...
	QueryBatchInsertIdentifiers = model.DBQuery{
		...
		Query: "INSERT INTO ENTITY_IDENTIFIER " +
			"(ENTITY_ID, TYPE, VALUE, SOURCE, DEPLOYMENT_ID) VALUES ",
	}
	...
	QueryAddIdentifier = model.DBQuery{
		...
		Query: "INSERT INTO ENTITY_IDENTIFIER (ENTITY_ID, TYPE, VALUE, SOURCE, DEPLOYMENT_ID) " +
			"VALUES ($1, $2, $3, $4, $5) ON CONFLICT (ENTITY_ID, DEPLOYMENT_ID, TYPE) " +
	...
	QueryRemoveIdentifier = model.DBQuery{
		...
		Query: "DELETE FROM ENTITY_IDENTIFIER WHERE ENTITY_ID = $1 AND TYPE = $2 AND DEPLOYMENT_ID = $3",
	}
)
...
func buildEntityCountQueryByOUIDs(
	...
	baseQuery := `SELECT COUNT(*) as total FROM ENTITY WHERE ENTITY_CATEGORY = $1`
	columnName := AttributesColumn

	args := []interface{}{category}

	if len(filters) > 0 {
		fq, filterArgs, err := buildFilterQueryWithOffset(queryID, baseQuery, columnName, filters, len(args))
		if err != nil {
...
func buildEntityListQueryByOUIDs(
	...
	baseQuery := `SELECT ENTITY_ID, OU_ID, ENTITY_CATEGORY, ENTITY_TYPE, STATE, ATTRIBUTES, SYSTEM_ATTRIBUTES ` +
		`FROM ENTITY WHERE ENTITY_CATEGORY = $1`
	columnName := AttributesColumn
...
func buildIdentifyQuery(filters map[string]interface{}, deploymentID string) (model.DBQuery, []interface{}, error) {
	baseQuery := "SELECT ENTITY_ID FROM ENTITY WHERE 1=1"
...
func buildBulkEntityExistsQuery(entityIDs []string, deploymentID string) (model.DBQuery, []interface{}, error) {
	return buildEntityINClauseQuery(
		"ASQ-ENTITY_MGT-20",
		"SELECT ENTITY_ID FROM ENTITY WHERE ENTITY_ID IN (%s) AND DEPLOYMENT_ID = %s",
...
func buildBulkEntityExistsQueryInOUs(
	...
	baseQueryTpl := "SELECT ENTITY_ID FROM ENTITY WHERE DEPLOYMENT_ID = %s AND OU_ID IN (%s) AND ENTITY_ID IN (%s)"
...
func buildEntityListQuery(
	...
	baseQuery := "SELECT ENTITY_ID, OU_ID, ENTITY_CATEGORY, ENTITY_TYPE, STATE, ATTRIBUTES, SYSTEM_ATTRIBUTES FROM ENTITY"
	queryID := "ASQ-ENTITY_MGT-22"
	columnName := AttributesColumn

	if len(filters) > 0 {
		baseWithCategory := baseQuery + " WHERE ENTITY_CATEGORY = $1"
		args := []interface{}{category}
		fq, fArgs, err := buildFilterQueryWithOffset(queryID, baseWithCategory, columnName, filters, len(args))
		if err != nil {
			return model.DBQuery{}, nil, err
		}
...
func buildIdentifyQueryFromIdentifiers(
	...
	baseQuery := `SELECT DISTINCT ia1.ENTITY_ID FROM ENTITY_IDENTIFIER ia1`
	whereConditions := []string{}
	args := []interface{}{}
	paramIndex := 1

	whereConditions = append(whereConditions, fmt.Sprintf("ia1.TYPE = $%d AND ia1.VALUE = $%d",
	...
	for i := 1; i < len(keys); i++ {
		...
		baseQuery += fmt.Sprintf(
			...
			alias, alias, alias)
		whereConditions = append(whereConditions, fmt.Sprintf("%s.TYPE = $%d AND %s.VALUE = $%d",
...
func buildIdentifyQueryHybrid(
	...
	postgresQuery := `SELECT DISTINCT e.ENTITY_ID FROM ENTITY e`
	postgresQuery += ` INNER JOIN ENTITY_IDENTIFIER ia1 ON e.ENTITY_ID = ia1.ENTITY_ID ` +
		`AND e.DEPLOYMENT_ID = ia1.DEPLOYMENT_ID`

	sqliteQuery := `SELECT DISTINCT e.ENTITY_ID FROM ENTITY e`
	sqliteQuery += ` INNER JOIN ENTITY_IDENTIFIER ia1 ON e.ENTITY_ID = ia1.ENTITY_ID ` +
		`AND e.DEPLOYMENT_ID = ia1.DEPLOYMENT_ID`
	...
	for i := 1; i < len(indexedKeys); i++ {
		...
		joinClause := fmt.Sprintf(
			" INNER JOIN ENTITY_IDENTIFIER %s ON e.ENTITY_ID = %s.ENTITY_ID "+
...
func buildGetEntitiesByIDsQuery(entityIDs []string, deploymentID string) (model.DBQuery, []interface{}, error) {
	return buildEntityINClauseQuery(
		"ASQ-ENTITY_MGT-26",
		"SELECT ENTITY_ID, OU_ID, ENTITY_CATEGORY, ENTITY_TYPE, STATE, ATTRIBUTES, SYSTEM_ATTRIBUTES "+
			"FROM ENTITY WHERE ENTITY_ID IN (%s) AND DEPLOYMENT_ID = %s",
		entityIDs, deploymentID,
	)
}
...
func buildPaginatedQuery(baseQuery string, paramCount int, placeholder string) (string, error) {
	switch placeholder {
	case "?":
		return fmt.Sprintf("%s ORDER BY ENTITY_ID LIMIT %s OFFSET %s",
...
// This is used when the base query already has parameters (e.g., ENTITY_CATEGORY = $1).
func buildFilterQueryWithOffset(
	queryID string, baseQuery string, columnName string, filters map[string]interface{}, paramOffset int,
) (model.DBQuery, []interface{}, error) {
	if err := utils.ValidateKey(columnName); err != nil {
	...
	// Replace $1 with ? for SQLite in the base query if needed
	if strings.Contains(sqliteQuery, "$1") {
		sqliteQuery = strings.Replace(sqliteQuery, "$1", "?", 1)
	}
