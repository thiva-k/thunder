...
type entityStoreInterface interface {
	...
	GetGroupCountForEntity(ctx context.Context, entityID string, category string) (int, error)
	GetEntityGroups(ctx context.Context, entityID string, category string, limit, offset int) ([]EntityGroup, error)
...
func (es *entityDBStore) GetGroupCountForEntity(ctx context.Context, entityID string, category string) (int, error) {
	dbClient, err := es.dbProvider.GetUserDBClient()
	if err != nil {
		return 0, fmt.Errorf("failed to get database client: %w", err)
	}

	countResults, err := dbClient.QueryContext(ctx, QueryGetGroupCountForEntity, entityID, category, es.deploymentID)
	if err != nil {
...
func (es *entityDBStore) GetEntityGroups(
	ctx context.Context, entityID string, category string, limit, offset int) ([]EntityGroup, error) {
	dbClient, err := es.dbProvider.GetUserDBClient()
	if err != nil {
		return nil, fmt.Errorf("failed to get database client: %w", err)
	}
	...
	results, err := dbClient.QueryContext(ctx, QueryGetGroupsForEntity,
		entityID, category, limit, offset, es.deploymentID)
	if err != nil {
