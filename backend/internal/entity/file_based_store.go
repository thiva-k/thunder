...
func newEntityFileBasedStore() (*entityFileBasedStore, transaction.Transactioner) {
	genericStore := declarativeresource.NewGenericFileBasedStore(entitystore.KeyTypeUser)
...
func (f *entityFileBasedStore) Create(id string, data interface{}) error {
	...
	return f.GenericFileBasedStore.Create(resource.Entity.EntityID, resource)
}
...
func (f *entityFileBasedStore) CreateEntity(ctx context.Context, entity Entity,
	...
	return f.GenericFileBasedStore.Create(entity.EntityID, resource)
}
...
// AddIdentifier is not supported in file-based store.
func (f *entityFileBasedStore) AddIdentifier(ctx context.Context, entityID, idType, value, source string) error {
	return errors.New("AddIdentifier is not supported in file-based store")
}

// RemoveIdentifier is not supported in file-based store.
func (f *entityFileBasedStore) RemoveIdentifier(ctx context.Context, entityID, idType string) error {
	return errors.New("RemoveIdentifier is not supported in file-based store")
}
...
func (f *entityFileBasedStore) IdentifyEntity(ctx context.Context,
	...
	for _, resource := range resources {
		if matchesFilters(resource.Entity.Attributes, filters) {
			matches = append(matches, resource.Entity.EntityID)
		}
	}
...
func (f *entityFileBasedStore) GetEntityListCount(ctx context.Context, category string,
	...
	count := 0
	for _, resource := range resources {
		if string(resource.Entity.EntityCategory) != category {
			continue
		}
		if matchesFilters(resource.Entity.Attributes, filters) {
			count++
