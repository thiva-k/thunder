...
func (c *entityCompositeStore) GetEntity(ctx context.Context, id string) (Entity, error) {
	return declarativeresource.CompositeGetHelper(
		...
		func() (Entity, error) { return c.fileStore.GetEntity(ctx, id) },
		ErrEntityNotFound,
	)
}
...
func (c *entityCompositeStore) GetEntityWithCredentials(ctx context.Context, id string) (
	...
	return c.fileStore.GetEntityWithCredentials(ctx, id)
}

// UpdateEntity updates an entity in the database store only.
...
// AddIdentifier adds an identifier in the database store only.
func (c *entityCompositeStore) AddIdentifier(ctx context.Context, entityID, idType, value, source string) error {
	return c.dbStore.AddIdentifier(ctx, entityID, idType, value, source)
}

// RemoveIdentifier removes an identifier from the database store only.
func (c *entityCompositeStore) RemoveIdentifier(ctx context.Context, entityID, idType string) error {
	return c.dbStore.RemoveIdentifier(ctx, entityID, idType)
}
...
func (c *entityCompositeStore) IdentifyEntity(ctx context.Context,
	...
	entityID, err := c.dbStore.IdentifyEntity(ctx, filters)
	if err == nil {
		return entityID, nil
	}
	if !errors.Is(err, ErrEntityNotFound) {
	...
	return c.fileStore.IdentifyEntity(ctx, filters)
}
...
func (c *entityCompositeStore) IsEntityDeclarative(ctx context.Context, id string) (bool, error) {
	...
	if err != nil {
		if strings.Contains(err.Error(), "not found") {
			isDeclarative, err = c.dbStore.IsEntityDeclarative(ctx, id)
...
func mergeAndDeduplicateEntities(dbEntities, fileEntities []Entity) []Entity {
	...
	for i := range dbEntities {
		...
		seen[dbEntities[i].EntityID] = true
	}

	for i := range fileEntities {
		if !seen[fileEntities[i].EntityID] {
			fileEntities[i].IsReadOnly = true
			result = append(result, fileEntities[i])
			seen[fileEntities[i].EntityID] = true
		}
	}
