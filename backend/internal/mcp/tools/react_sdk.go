...
func (t *ReactSDKTools) RegisterTools(server *mcp.Server) {
	// Register integrate_react_sdk tool
	schema := GenerateSchema[IntegrateReactSDKInput]()
	mcp.AddTool(server, &mcp.Tool{
		Name:        "integrate_react_sdk",
		Description: "Provides instructions and code snippets for integrating Thunder via the Asgardeo React SDK into a React application.",
		InputSchema: schema,
...
type IntegrateReactSDKInput struct {
	...
	AppURL     string `json:"app_url,omitempty" jsonschema:"URL of the React application for redirect"`
}
...
func (t *ReactSDKTools) IntegrateReactSDK(
	...
	// Detailed instructions from the guide
	// Note: We use `~` instead of backticks in this raw string literal to avoid syntax errors,
	// then replace them at runtime.
	rawInstructions := `
# Thunder Authentication – React Integration Instructions (Inbuilt Mode)

## Project Context
This project is a **React application** that integrates **Thunder authentication** using the **Asgardeo React SDK** with **Thunder-hosted (inbuilt) login pages**.
...
- **Client ID**: P9FvHHcAaBArc1dXyFyC8A
...
Always create a NEW public application with token endpoint method as none via create_app tool and use the client id.
