...
//
// The Control Plane is where configuration is authored and validated. It serves the management
// APIs and the administration console, and it serves no runtime traffic: no token issuance, no
// login or registration, no flow execution. Which services that means is in servicemanager.go.
//
// The start and stop sequence mirrors cmd/server, deliberately as its own copy so that changing
// how the runtime server starts cannot change how the Control Plane starts.
package main
...
// Console frontend path plus any configured extras. Empty and root ("/") entries are
...
func createHTTPServer(ctx context.Context, logger *log.Logger, cfg *config.Config, mux *http.ServeMux,
	...
	// The Console frontend path is always excluded from the access log to keep it
...
// registerStaticFileHandlers registers static file handlers for frontend applications. The
// Control Plane serves only the console: the Gate login application belongs to a data plane.
