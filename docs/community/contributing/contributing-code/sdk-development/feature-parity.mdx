...
The check posts one comment carrying a checklist of the sibling repositories, and keeps it up to date as you work. **Reply to that comment** and the boxes tick themselves. Nothing needs editing by hand, which matters for contributors who cannot edit a comment the bot owns.

Link a port in either style:

```text
thunder-id/ios-sdks#412
https://github.com/thunder-id/ios-sdks/pull/412
```

A linked issue counts too, so a deferred port is recorded rather than forgotten:

```text
Deferred for Android: thunder-id/android-sdks#233
```

For an SDK the change does not reach, name the repository and say so on one line:

```text
thunder-id/flutter-sdks: not applicable, it wraps the native SDKs and needs no change
```

Any of `not applicable`, `n/a`, `not needed`, `no change needed`, or `does not apply` marks it. Write the reason on the same line, since the next person reading the pull request needs to know why, not just that somebody decided.
