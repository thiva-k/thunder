...
Download and extract the <ProductName /> distribution. Follow [Get <ProductName />](../../../../getting-started/get-thunderid) for the steps.
...
Deploy <ProductName /> to any cloud or on-premise host where it is reachable over HTTPS. See [Deployment Patterns](../../../../deployment/production-guidelines) for guidance.
...
See [Configuration](../../../../deployment/configuration) for a full reference.
...
Point the apps at your public URL by setting it in their `.env` files:

- In `frontend/.env`, set `VITE_THUNDER_BASE_URL=<PUBLIC_URL>`.
- In `backend/.env` and `lounge/.env`, set `THUNDER_BASE_URL=<PUBLIC_URL>`.
...
- Ready for production? See the [Production Deployment Guidelines](../../../../deployment/production-guidelines).
