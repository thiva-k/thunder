...
- Set `ALLOWED_TIERS=Platinum` in `lounge/.env` and restart the lounge, then re-verify John's `Gold` pass: access is now denied, decided entirely on the verified claim.
