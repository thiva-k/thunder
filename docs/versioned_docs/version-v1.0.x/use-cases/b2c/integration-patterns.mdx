...
Wayfinder uses a redirect-based pattern: sign-in, sign-up, and recovery all run as hosted <ProductName /> screens. That
fits a browser-based travel app well, but it is not the only option. The integration pattern determines where identity
screens live and who controls each step in the journey. The right choice depends on your user experience goals,
development resources, and how much control you want over the journey.
