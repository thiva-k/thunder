...
<ProductName /> signs every token (access token, ID Token, and signed UserInfo response) with the deployment signing key, and the algorithm follows that key's type, for example an RSA key signs with `RS256` and an Ed25519 key signs with `EdDSA`.

To see which algorithm a deployment signs with, read `id_token_signing_alg_values_supported` (and `userinfo_signing_alg_values_supported`) in the [Server Metadata](../server-metadata) document, or inspect the published keys at the [JWKS](../jwks) endpoint. Clients verify signatures against those keys.
...
3. Use the **ID Token** section to configure ID Token format and, for encrypted formats, encryption. Signing uses the deployment signing key.
