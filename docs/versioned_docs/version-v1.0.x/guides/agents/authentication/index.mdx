...
The agent authenticates to <ProductName /> with one of these methods. This credential is what ties every request back to that specific agent, so choose the strength that matches how sensitive the agent's work is.
