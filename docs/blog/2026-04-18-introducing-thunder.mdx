...
<ProductName /> is written in Go: high performance, low latency, small runtime footprint. It is headless and API-first, with built-in GitOps support.
...
Beyond SDKs, <ProductName /> provides a console UI, RESTful APIs, server-side and client-side SDKs, and a flow engine and a visual flow designer for building login, registration, account recovery, and step-up authentication experiences without implementing each flow from scratch.
...
<ProductName /> is in active development and openly built. The best way to understand what it is and where it's going is to try it. 
