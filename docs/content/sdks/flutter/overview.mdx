...
To get started quickly, follow the [Flutter Quickstart Guide](../../../getting-started/connect-your-application/flutter) for step-by-step setup instructions.
...
| `BaseUserProfile(onSaved, onError, builder)` | Builder variant with full control over the form layout |

#### Guards

| Widget | Description |
|--------|-------------|
| `SignedIn(child, fallback)` | Renders `child` only when the user is authenticated |
| `SignedOut(child, fallback)` | Renders `child` only when the user is not authenticated |
