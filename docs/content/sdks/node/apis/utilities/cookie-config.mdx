---
title: CookieConfig
description: Default cookie configuration constants for the Node.js SDK
---

# `CookieConfig`

`CookieConfig` is a constants class that exposes the default values used for session cookie configuration in the <ProductName /> Node.js SDK. Use these constants when setting cookies for session management or when passing options to [`getSessionCookieOptions`](/docs/next/sdks/node/apis/utilities/get-session-cookie-options).
...
- [`CookieOptions`](/docs/next/sdks/node/apis/utilities/cookie-options) — Interface describing cookie option fields
- [`getSessionCookieOptions()`](/docs/next/sdks/node/apis/utilities/get-session-cookie-options) — Helper to build cookie options using these defaults
