...
The `useThunderID` composable provides access to the <ProductName /> authentication context in Vue applications. It allows you to retrieve authentication state, user information, and other context values managed by the [`<ThunderIDProvider />`](/docs/next/sdks/vue/apis/providers/thunderid-provider).
...
| `http` | `{ request, requestAll }` | Authenticated HTTP client (see [Accessing Protected APIs](/docs/next/sdks/vue/guides/accessing-protected-apis)) |
