...
description: Read the session or throw a 401 error
---
...
Returns a `Promise<ThunderIDSessionPayload>`. See [`useServerSession()`](/docs/next/sdks/nuxt/apis/server/use-server-session#thunderidsessionpayload) for the full payload shape.
...
- Use [`useServerSession()`](/docs/next/sdks/nuxt/apis/server/use-server-session) when you want to handle the unauthenticated case yourself instead of throwing automatically.
- If the access token in the session may be expired, use [`getValidAccessToken()`](/docs/next/sdks/nuxt/apis/server/get-valid-access-token) after calling `requireServerSession` to ensure you have a fresh token.
