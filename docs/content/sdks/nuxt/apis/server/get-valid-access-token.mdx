...
description: Get a valid access token, refreshing it automatically if expired
---
...
- To access session metadata alongside the token, call [`requireServerSession()`](/docs/next/sdks/nuxt/apis/server/require-server-session) first and use `getValidAccessToken` only when you need to forward the token to an external API.
