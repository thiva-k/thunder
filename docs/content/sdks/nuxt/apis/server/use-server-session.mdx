...
description: Read the session payload from the current request
---
...
- Use [`requireServerSession()`](/docs/next/sdks/nuxt/apis/server/require-server-session) to throw a `401` automatically instead of handling `null` manually.
- The session payload is also available synchronously via [`getThunderIDContext()`](/docs/next/sdks/nuxt/apis/server/get-thunderid-context) if the SSR plugin has already run.
