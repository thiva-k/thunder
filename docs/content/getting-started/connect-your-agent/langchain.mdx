...
2. Fill in the **agent attributes** the schema asks for. With the default schema these are:
    ...
    - **Model** (required): the model name, e.g. `gemini-flash-lite-latest`.
...
# Optional. Defaults to Gemini Flash Lite; set MODEL_NAME to override, e.g.
# google_genai:gemini-flash-lite-latest on Python, google-genai:gemini-flash-lite-latest on Node.
# MODEL_NAME=google_genai:gemini-flash-lite-latest
```
...
1. On the **Advanced** tab, under **Operating Mode**, turn on the **Delegated mode** toggle. This extends the agent to also sign users in, enabling the `authorization_code` grant the on-behalf-of flow needs.
2. On the **Advanced** tab, set the **Allowed user types** (for example, **Person**) so users of that type can sign up through this agent, and under **Redirect URIs** add `http://localhost:6274/callback` (this must match `REDIRECT_URI` in your `.env`).
3. Click **Save**.
...
MODEL_NAME      = os.environ.get("MODEL_NAME", "google_genai:gemini-flash-lite-latest")
...
const MODEL_NAME      = process.env.MODEL_NAME ?? "google-genai:gemini-flash-lite-latest";
...
<NextSteps>
  ...
  />
  <NextStepsCard
    title="Create and Manage Your Agent"
    description="Go deeper on the agent you registered: manage its owner, attribute schema, and lifecycle from the Console."
    href="../../../guides/agents/manage-agents"
  />
</NextSteps>
