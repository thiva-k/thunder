...
2. On the **Advanced** tab, set the **Allowed user types** (for example, **Person**) so users can authenticate through this agent, and under **Redirect URIs** add `http://localhost:6274/callback` (this must match `REDIRECT_URI` in your `.env`).
3. Click **Save**.
