...
<TutorialHeroItem icon={<Server />}>Node.js 22.19+ (for MCP Inspector)</TutorialHeroItem>
...
<ProductName /> supports two ways for a client to obtain OAuth credentials: pre-register an application in the Console, which MCP Inspector uses below, or let the client register itself automatically through Dynamic Client Registration, which Claude Code uses.
...
MCP Inspector connects with a client application you pre-register in the Console.
...
Claude Code registers its own OAuth client automatically through Dynamic Client Registration (DCR), instead of using a pre-registered application.
