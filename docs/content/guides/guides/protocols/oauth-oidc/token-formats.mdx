...
Encrypted responses (`JWE`, `NESTED_JWT`) and `private_key_jwt` client authentication require a certificate on the application — either an inline `JWKS` or a `JWKS_URI` <ProductName /> can fetch. See [Application certificate](../client-authentication-methods#application-certificate) for the full rules.
...
- [Client Authentication Methods — Application certificate](../client-authentication-methods#application-certificate) — attaching the client's public key
