...
```json
{
  "id": "send-otp",
  "type": "TASK_EXECUTION",
  "executor": "OTPNotificationSender",
  "properties": {
...
| **Send SMS OTP** | Sends an OTP to the user's registered mobile number. | SMS sender configured |
| **Verify SMS OTP** | Verifies the OTP code the user entered. | Send SMS OTP must have run |
...
- `relyingPartyId` node property must be set — the domain where the passkey is used (e.g., `app.example.com`). The executor hard-fails if missing.
...
| `relyingPartyName` | Relying Party Name | No | Display name of the relying party (e.g. `ThunderID`) |
...
- `relyingPartyId` node property must be set — the domain where passkeys will be registered (e.g., `app.example.com`). The executor hard-fails if missing.
...
**Prerequisites:**
- An SMS sender must be configured in <ProductName />
- `senderId` node property must be set — the executor hard-fails if `senderId` is not configured
...
In registration flows, the executor prompts for the mobile number if not already available and checks for uniqueness — failing if the number is already registered.

**Input Configuration:** None. The mobile number is resolved from prior inputs or user store. This executor has no registered node input default.
...
**Prerequisites:**
- `otpSessionToken` (required) — set in runtime data by Send SMS OTP. Not a configurable node input. The executor fails if absent.
- An SMS sender must be configured in <ProductName />

**View pairing:** Must be immediately preceded by the **SMS OTP View**, which collects the OTP code the user received.
...
- An email service must be configured in <ProductName /> (Console > Settings > Notifications > Email). The executor returns failure status if no email client is configured.
...
**Prerequisites:**
- An SMS sender must be configured in <ProductName /> (Console > Settings > Notifications > SMS)
...
**Prerequisites:** None. Any context data referenced via `{{fieldName}}` placeholders in the URL or body must be available from prior executors at runtime.

**Input Configuration:** None. This executor is entirely configuration-driven; all settings are node properties.

**How it works:**
1. Parses HTTP configuration from node properties (URL, method, headers, body, timeout)
2. Resolves template placeholders in the URL and body using data from prior executors (e.g., `{{userId}}`, `{{email}}`)
...
| Property | UI Label | Required | Default | Description |
|---|---|---|---|---|
| `url` | URL | Yes | — | The endpoint URL (supports placeholders like `{{userId}}` from prior executors) |
