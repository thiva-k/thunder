...
<ProductName /> intersects the requested scopes against the scopes authorized for the agent via its group memberships and role assignments. Requesting a scope the agent is not authorized for results in `invalid_scope`.
...
The issued token represents the agent acting on behalf of the user. Its scopes are bounded by the scopes present in the `subject_token` — the agent cannot escalate the user's permissions.
...
### Narrow to a Specific Audience or Resource

Use `audience` for a named target service, or `resource` (RFC 8707) for a resource server URI:

```bash
  -d 'resource=https://api.example.com/invoices'
```

If both the `subject_token`'s audiences and the requested `resource` are specified, <ProductName /> merges them per RFC 8693 §2.1.
...
Refresh tokens preserve the original full set of authorized audiences, so later narrowing with `resource` still works across refreshes.
...
- [Resource Indicators](../../resource-indicators) — Use RFC 8707 `resource` parameters to issue audience-restricted tokens.
