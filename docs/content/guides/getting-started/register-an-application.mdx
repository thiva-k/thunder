...
9. On the **Configuration** screen, set the URL where your application is hosted:
   ...
   <Tabs groupId="app-type">
     <TabItem value="quickstart" label="React or Vue Quickstart" default>
     ...
     <TabItem value="sample" label="Sample App">

     Set **Where is your application hosted?** to `http://localhost:3000`.

     The ThunderID sample app runs on port `3000` by default.

     </TabItem>
     <TabItem value="custom" label="My Own App">

     Set **Where is your application hosted?** to the root URL of your app (for example, `http://localhost:3000`).

     Leave **After Sign-in URL** set to **Same as Application Access URL** unless your app handles the callback at a specific path such as `/callback`. In that case, choose **Custom URL** and enter the full callback URL.
