...
1. On the **Advanced** tab, set **Operating Mode** to **On behalf of a user**. The agent keeps acting on its own (`client_credentials`) and gains the ability to sign a user in and act for them (`authorization_code` with PKCE).
2. Still on the **Advanced** tab, add the **Authorized redirect URIs** for the sign-in callback, and restrict **Allowed User Types** if only certain user types may sign in through this agent.
