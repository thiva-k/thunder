...
1. The `thunderid-oc-resourcetype` manifest registers the `thunderid` `ClusterResourceType`, which holds the schema and the Kubernetes templates.
...
The steps below run from a local checkout of the <ProductName /> repository, which provides the `ResourceType` manifest, the sample `Resource`, and (for `postgres` mode) the database schema scripts. Clone it and run the remaining commands from its root:

```bash
git clone https://github.com/thunder-id/thunderid.git
cd thunderid
```

## Install the ResourceType

A platform administrator applies the manifest once per cluster:

```bash
kubectl apply -f install/openchoreo/thunderid-oc-resourcetype/resourcetype.yaml
```

This registers the cluster-scoped `thunderid` `ClusterResourceType`, shared across all projects.
...
Use the sample manifests at `install/openchoreo/thunderid-oc-resourcetype/samples/resource.yaml` as a starting point. The sample ships with `<SERVER_PUBLIC_URL>` and `<GATEWAY_HOSTNAME>` placeholders that must be substituted before applying.

:::warning
Do not `kubectl apply` the sample unedited. The unsubstituted placeholders are invalid values (for example an invalid CORS origin and redirect URI), and the pod crash-loops at startup. Always substitute the placeholders first.
:::

Substitute the placeholders and apply in one step. The values below are the OpenChoreo quick-start defaults, where `serverPublicUrl` follows the gateway hostname pattern `<environmentName>-<resourceName>-<resourceNamespace>.<gateway-host>`:

```bash
sed -e 's|<SERVER_PUBLIC_URL>|http://development-thunderid-default.openchoreoapis.localhost:19080|g' \
    -e 's|<GATEWAY_HOSTNAME>|development-thunderid-default.openchoreoapis.localhost|g' \
    install/openchoreo/thunderid-oc-resourcetype/samples/resource.yaml | kubectl apply -f -
```

Adjust the two values for your own gateway host if you are not using the quick-start.
...
:::note
Exports strip user credentials. The sample already adds a `credentials:` block for the admin user; add one to any other user that must sign in, and resolve the password from the secret store so it never appears in the manifest:
...
For the full parameter reference, see the documentation at `install/openchoreo/thunderid-oc-resourcetype/README.md`.
