...
The Sky Pass claims come from the credential subject's profile, so the tryout needs a `Customer` user type with the loyalty attributes, and a test user (John Doe) whose values become the issued credential's claims.
...
```bash
curl -k -X POST https://localhost:8090/applications \
  ...
  -d '{
    ...
    "inboundAuthConfig": [{
      ...
      "config": {
        ...
        "redirectUris": ["ch.ubique.funke://issuance"],
...
Repeat for **Lissi** with `clientId` `9c481dc3-2ad0-4fe0-881d-c32ad02fe0fc` and redirect URI `https://oob.lissi.io/vci-cb`.
