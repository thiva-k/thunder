...
Add the Wayfinder and Skyline Lounge origins to the server-config `cors` section so the browser apps can call <ProductName />. Create or update `config/resources/server_configs/cors.yaml`:
