...
Other times, the answer depends on context. An agent responding to a user's request should only access what that user can access. If a junior employee asks the agent to pull a report, the agent should not return data the employee is not authorized to see. This remains true even if the agent itself has broader access. The service needs to know "this is Agent X acting on behalf of User Y, and User Y is allowed to read this data."
