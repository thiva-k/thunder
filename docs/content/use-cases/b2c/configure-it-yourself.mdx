...
Use this page to build the Wayfinder setup manually instead of importing the declarative bundle from [Try It Out](../try-it-out). Pick this path when you want to see exactly what gets created, adapt the setup to your own project, or learn the moving parts step by step.
...
- [Build the Registration Flow](#build-the-registration-flow): needed for [Self Sign-Up](../try-it-out/self-sign-up).
- [Build the Account Recovery Flow](#build-the-account-recovery-flow): needed for [Account Recovery](../try-it-out/account-recovery).
- [Set up Internal User Onboarding](#set-up-internal-user-onboarding): needed for [Onboard Internal Users](../try-it-out/onboard-internal-users).
...
SMTP must be configured for recovery emails to reach the user. See [Configure SMTP Server](../../../guides/guides/smtp-server/smtp-server-configuration) for setup instructions. This is required regardless of which path you chose.
...
These resources drive [Onboard Internal Users](../try-it-out/onboard-internal-users).
