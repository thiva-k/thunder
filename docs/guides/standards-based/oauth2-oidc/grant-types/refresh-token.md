...
The Refresh Token flow allows clients to obtain new access tokens using a refresh token, without requiring the user to re-authenticate. This is essential for maintaining long-lived sessions and improving user experience.

## Overview

When an access token expires, instead of redirecting the user through the full authentication flow again, you can use a refresh token to obtain a new access token.
...
2. **Refresh token** obtained from initial authorization (Authorization Code flow)

## Step-by-Step Guide

### Step 1: Obtain Refresh Token

First, obtain a refresh token through the Authorization Code flow. The refresh token is included in the token response:
...
|-------|-------------|----------|
| `invalid_grant` | Invalid or expired refresh token | Obtain a new refresh token through authorization flow |
...
- [Authorization Code Flow](authorization-code.md) - How to obtain initial refresh token
