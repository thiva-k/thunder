...
The Client Credentials flow is used for machine-to-machine (M2M) communication where no user interaction is required. It's ideal for service-to-service authentication and backend API access.

## Overview

The Client Credentials flow allows applications to authenticate using their client credentials (client ID and secret) to obtain an access token directly, without user involvement.
...
Create an application with OAuth 2.0 configuration supporting the client credentials flow:
