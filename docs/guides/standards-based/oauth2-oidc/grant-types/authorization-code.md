...
The Authorization Code flow involves redirecting users to an authorization server where they authenticate and authorize the application, then exchanging an authorization code for an access token.

## Overview

The Authorization Code flow is the recommended OAuth 2.1 flow for applications that require user authentication. It can be used with both confidential clients (with client secrets) and public clients (with PKCE).
...
Create an application with OAuth 2.0 configuration supporting the authorization code flow:
...
curl -kL -X POST https://localhost:8090/applications \
  ...
  -d '{
    ...
    "description": "Web application using authorization code flow",
...
Thunder supports PKCE (Proof Key for Code Exchange) for enhanced security, mandatory for public clients adhering to OAuth 2.1.
...
See the [OAuth Sample Application](../../../../../samples/apps/oauth/) for a complete implementation example.

## Related Documentation

- [PKCE Implementation](../features/pkce.md) - Detailed PKCE guide
- [Refresh Token Flow](refresh-token.md) - Using refresh tokens
