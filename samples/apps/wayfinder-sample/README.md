...
The importable bundle already adds `http://localhost:5173` to the server-config `cors` section. If you serve the frontend from a different host or port, update the `# resource_type: server_config` document in `thunderid-config/redirect/thunderid-config.yaml` before importing, or use `PUT /server-config/cors` after startup.
...
1. Register an SMS notification sender via the ThunderID API:
   ...
   Pass your SMS provider credentials in the request body. Copy the returned sender `id` into `thunderid-config/redirect/thunderid.env`:
...
| `CIBA_HINT_TYPE` | `login_hint` | Hint type sent to `bc-authorize`. `login_hint` uses the user's email address; `id_token_hint` uses the user's OIDC ID token stored at upgrade-request time. Changing this also requires updating `CIBA_LOGIN_HINT_ATTRIBUTE` in `thunderid-config/redirect/thunderid.env` and re-importing the ThunderID config. |
