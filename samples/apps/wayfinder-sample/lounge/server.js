...
// through Thunder's OpenID4VP API, and grants/denies entry based on the verified
// tier. No OAuth client registration is needed — the request object is signed by
// Thunder's verifier key (x509_san_dns), which the wallet trusts.
...
// so one env var (VITE_THUNDER_BASE_URL) points the whole sample at Thunder.
...
// Local-dev TLS bypass: Thunder ships a self-signed cert on localhost. Only
...
if (/^https?:\/\/(localhost|127\.0\.0\.1)(:\d+)?(\/|$)/.test(THUNDER_BASE_URL)) {
  ...
  console.warn("[lounge] Local Thunder detected — TLS verification disabled. Dev only.");
}
...
async function thunderFetch(path, options = {}) {
  ...
  if (!response.ok) {
    const error = new Error(body.error || body.message || `Thunder request failed (${response.status})`);
...
const server = createServer(async (request, response) => {
  try {
    ...
    // Start a verification: ask Thunder for a presentation request the wallet can scan.
...
server.listen(PORT, () => {
  ...
  console.log(`  Thunder    : ${THUNDER_BASE_URL || "(set THUNDER_BASE_URL)"}`);
