...
interface AuthResponse {
    flowStatus?: string;
    // The raw assertion never reaches the browser; the /api/flow proxy captures it into an
    // httpOnly cookie and reports its presence via this flag instead. See src/app/api/flow/route.ts.
    assertionIssued?: boolean;
...
const LoginPage = () => {
    ...
    const { completeAuthentication, resetAuthState } = useAuth();
    ...
    const processAuthResponse = useCallback((data: AuthResponse, selectedAction?: string, autoRedirect: boolean = false) => {
        ...
        if (data.flowStatus && data.flowStatus === 'COMPLETE') {
            ...
            if (isCameFromDecision) {
                ...
                // Pass the new challenge token directly, since the React state update for
        ...
        setLoading(false);
    }, [needsDecision, isRecoveryMode, isSignupMode, resetAuthState, completeAuthentication, handleSocialLoginClick]);

    // Handle when user selects an authentication option
    const handleAuthOptionSelection = (actionId: string) => {
        // Signup and forgot-password are mode switches, not steps within the current
        // authentication flow: they must restart with the corresponding flow type (matching the
        // "Sign up!" / "Sign in!" links below) rather than continue the in-progress executionId,
        // for two reasons. First, isSignupMode/isRecoveryMode drive the page's headings, button
        // labels, and bottom links, and only get set on a fresh init()/initRecovery() call, not on
        // a decision submitted against the existing execution. Second, initRecovery() calls
        // initiateNativeAuthFlow('RECOVERY'), which is the only place the server checks whether
        // recovery is enabled for the application; continuing the existing (login) execution
        // through this same decision does not.
        if (actionId.includes('signup') || actionId.includes('sign_up')) {
        ...
        if (actionId.includes('forgot_password')) {
            setError(false);
            setErrorMessage('');
            initRecovery();
            return;
    ...
    }, [resetAuthState, processAuthResponse, completeAuthentication]);
    ...
    // Human-readable label for actions that aren't social/mobile continuations (e.g. links to
    // switch mode, like "forgot password" or "sign up"). See docs/REFERENCE.md's UI Rendering
    // and Action Ref Convention section.
    const getOtherActionLabel = (ref: string): string => {
        if (ref.includes('forgot_password')) {
            return 'Forgot Password?';
        }
        if (ref.includes('signup') || ref.includes('sign_up')) {
            return 'Create Account';
        }
        if (ref.includes('signin') || ref.includes('sign_in')) {
            return 'Sign In';
        }
        return ref;
    };
                            ...
                            <Box>
                                ...
                                {otherActions.map((action, index) => (
                                    <Button
                                        ...
                                        sx={{ my: 1 }}
                                    >
                                        {getOtherActionLabel(action.ref) || 'Continue'}
                                    </Button>
                                ))}
                            </Box>
                        )}
                    ...
                    {availableActions.length > 1 && (
                         <Box sx={{ mt: 1 }}>
                            ...
                            {availableActions.slice(1).map((action, index) => {
                                ...
                                let label = getOtherActionLabel(action.ref || "Continue");
                                if (isSocial) {
